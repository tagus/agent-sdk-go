...
func WithBaseURL(baseURL string) Option {
	return func(c *OpenAIClient) {
		...
		c.ResponseService = c.Client.Responses
	}
}

// NewClient creates a new OpenAI client
func NewClient(apiKey string, options ...Option) *OpenAIClient {
	openaiClient := openai.NewClient(option.WithAPIKey(apiKey), option.WithBaseURL("https://api.openai.com/v1"))

	// Create client with default options
	client := &OpenAIClient{
		Client:          openaiClient,
		ChatService:     openai.NewChatService(option.WithAPIKey(apiKey), option.WithBaseURL("https://api.openai.com/v1")),
		ResponseService: openaiClient.Responses,
		Model:           "gpt-4o-mini",
		apiKey:          apiKey,
